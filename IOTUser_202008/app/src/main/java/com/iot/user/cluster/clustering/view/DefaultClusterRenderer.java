package com.iot.user.cluster.clustering.view;
...
import com.iot.user.cluster.MarkerManager;
...
import com.iot.user.cluster.projection.Point;
import com.iot.user.cluster.projection.SphericalMercatorProjection;
import com.iot.user.cluster.ui.IconGenerator;
import com.iot.user.cluster.ui.SquareTextView;
...
import static com.iot.user.cluster.clustering.algo.NonHierarchicalDistanceBasedAlgorithm.MAX_DISTANCE_AT_ZOOM;
...
public class DefaultClusterRenderer<T extends ClusterItem> implements
    ...
    //修改默认地图气泡显示
    private static final int[] BUCKETS = {100, 200, 500, 1000};//{10, 20, 50, 100, 200, 500, 1000};
     ...
     * Icons for each bucket.
     */
    private SparseArray<BitmapDescriptor> mAlertIcons = new SparseArray<BitmapDescriptor>();

    /**
     * Icons for each bucket.
     */
    private SparseArray<BitmapDescriptor> mWarnIcons = new SparseArray<BitmapDescriptor>();
    ...
    public DefaultClusterRenderer(Context context, AMap map, ClusterManager<T> clusterManager) {
        mMap = map;
    ...
    @Override
    public void onAdd() {
        mClusterManager.getMarkerCollection().setOnMarkerClickListener(new AMap.OnMarkerClickListener() {
            @Override
        ...
        mClusterManager.getClusterMarkerCollection().setOnMarkerClickListener(new AMap.OnMarkerClickListener() {
            @Override
            public boolean onMarkerClick(Marker marker) {
    ...
    private class RenderTask implements Runnable {
        ...
        public void run() {
            ...
            final LatLngBounds visibleBounds = mMap.getProjection().getMapBounds(mMap.getCameraPosition().target,mMap.getCameraPosition().zoom);
            ...
            for (Cluster<T> c : clusters) {
                ...
                if(visibleBounds!=null){
                    onScreen = visibleBounds.contains(c.getPosition());
                }
    ...
    protected void onBeforeClusterRendered(Cluster<T> cluster, MarkerOptions markerOptions) {
        ...
        for(T t :cluster.getItems()){
            if(state == Constants.DEV_COMPLEX || state == Constants.DEV_ALARM){
                break;
            }
            if(state < t.getDevState()){
                state = t.getDevState();
            }
        ...
        if(state == Constants.DEV_WARN){
            color = Color.argb(255,255,215,0);
            descriptor = mWarnIcons.get(bucket);
        }else if(state == Constants.DEV_ALARM){
            color = Color.RED;
            descriptor = mAlertIcons.get(bucket);
        }else if(state == Constants.DEV_COMPLEX){
            color = Color.RED;
            descriptor = mAlertIcons.get(bucket);
        }else{
            color = Color.BLUE;
            descriptor = mIcons.get(bucket);
        }
...
//            mColoredCircleBackground.getPaint().setColor(getColor(bucket));
            ...
            descriptor = BitmapDescriptorFactory.fromBitmap(mIconGenerator.makeIcon(getClusterText(bucket)));//默认颜色
//            descriptor = BitmapDescriptorFactory.fromBitmap(mIconGenerator.makeBackGroundColor(getClusterText(bucket),color));//根据是否有报警设置颜色
            if(state == 1){
                mWarnIcons.put(bucket, descriptor);
            }else if(state == 2){
                mAlertIcons.put(bucket, descriptor);
            }else{
                ...
                for (T item : cluster.getItems()) {
                    ...
                    if (marker == null) {
                        ...
                        if (animateFrom != null) {
                            ...
                            markerOptions.title(item.getTitle()==null?"":item.getTitle()).snippet(item.getSnippet()==null?"":item.getSnippet());
                        } else {
                        ...
                        /****报警或者故障设备添加闪烁***/
                        int state = item.getDevState();
                        if(state == 1 || state == 2){
                            //设置锚点动画
                            Animation markerAnimation = new ScaleAnimation(0.5f, 1.3f, 0.5f, 1.3f); //初始化生长效果动画
                            markerAnimation.setDuration(1000);  //设置动画时间 单位毫秒
                            markerAnimation.setRepeatCount(Integer.MAX_VALUE);
                            marker.setAnimation(markerAnimation);
                            marker.startAnimation();
                        }else{
                            AlphaAnimation mADDAnimation=new AlphaAnimation(1, 1);
                            mADDAnimation.setRepeatCount(Integer.MAX_VALUE);
